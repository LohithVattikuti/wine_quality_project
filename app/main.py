...
# Load the model
model_path = "./outputs/stacked_model_no_xgboost.joblib"  # Adjusted model path
try:
...
# Function to ensure input features match the model's expectations
def ensure_13_features(features_array):
    """
    Adjusts the input array to 13 features by appending default values for the missing features.
    """
    expected_features = 13
    current_features = features_array.shape[1]

    if current_features < expected_features:
        # Add two zeros (or any default values) to make it 13 features
        missing_features = np.zeros((features_array.shape[0], expected_features - current_features))
        features_array = np.hstack((features_array, missing_features))
...
def predict(features: WineFeatures):
    ...
    try:
        ...
        # Ensure input has 13 features
        feature_values = ensure_13_features(feature_values)
...
def predict_batch(batch: BatchWineFeatures):
    ...
    try:
        ...
        # Ensure input has 13 features
        batch_features = ensure_13_features(batch_features)
